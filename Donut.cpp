...
// Donut.cpp is a class that extends the Game class ti impleent the game rules when being played in classic mode.
// Donut.cpp has many calls to other methods (such as playInside and playOutside) that are in the game class and used by every game mode
...
char** Donut::playDonut(char** currGen, int numRows, int columns){
  ...
  //checks for the wrapped around cells
  ...
  //checking for wrapped around cells
  if(currGen[0][0]== 'X'){
    ++bottomRightCorner;
  }
  if(currGen[0][cols-1]== 'X'){
